	...
	implements ICFBamISOCtryCcyObj
{
	...
	public ICFSecISOCtryCcyObj realise() {
		ICFSecISOCtryCcyObj retobj = ((ICFBamSchemaObj)getSchema()).getISOCtryCcyTableObj().realiseISOCtryCcy(
	...
	@Override
	public void forget() {
		((ICFBamSchemaObj)getSchema()).getISOCtryCcyTableObj().reallyDeepDisposeISOCtryCcy( (ICFSecISOCtryCcyObj)this );
	}

	@Override
	public ICFSecISOCtryCcyObj read() {
		ICFSecISOCtryCcyObj retobj = ((ICFBamSchemaObj)getSchema()).getISOCtryCcyTableObj().readISOCtryCcyByIdIdx( getPKey().getRequiredISOCtryId(),
	...
	public ICFSecISOCtryCcyTableObj getISOCtryCcyTable() {
		return( ((ICFBamSchemaObj)getSchema()).getISOCtryCcyTableObj() );
	}
	...
	public ICFSecISOCtryCcy getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFSecBackingStore().getTableISOCtryCcy().readDerivedByIdIdx( ((ICFBamSchemaObj)getSchema()).getAuthorization(),
	...
	public ICFSecISOCtryCcyEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFBamSchemaObj)getSchema()).getISOCtryCcyTableObj().lockISOCtryCcy( getPKey() );
		}
		edit = ((ICFBamSchemaObj)getSchema()).getISOCtryCcyTableObj().newEditInstance( lockobj );
	...
	public ICFSecISOCcyObj getRequiredParentCcy( boolean forceRead ) {
		if( ( requiredParentCcy == null ) || forceRead ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				requiredParentCcy = ((ICFBamSchemaObj)getSchema()).getISOCcyTableObj().readISOCcyByIdIdx( getPKey().getRequiredISOCcyId(), forceRead );
			}
		}
