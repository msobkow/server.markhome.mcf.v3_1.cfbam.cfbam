// Description: Java 25 edit object instance implementation for CFBam TZDateDef.
	...
	implements ICFBamTZDateDefEditObj
{
	...
	public int getClassCode() {
		return( ((ICFBamSchemaObj)orig.getSchema()).getTZDateDefTableObj().getClassCode() );
	}
	...
	public ICFBamValue getRec() {
		if( rec == null ) {
			rec = getOrigAsTZDateDef().getSchema().getCFBamBackingStore().getCFBamFactory().getFactoryTZDateDef().newRec();
			rec.set( orig.getRec() );
		}
		return( rec );
	}
	...
	public void setOptionalDummy( String value ) {
		if( getTZDateDefRec().getOptionalDummy() != value ) {
			getTZDateDefRec().setOptionalDummy( value );
		}
	}

	@Override
	public void copyRecToOrig() {
		ICFBamTZDateDef origRec = getOrigAsTZDateDef().getTZDateDefRec();
		ICFBamTZDateDef myRec = getTZDateDefRec();
		origRec.set( myRec );
	}
