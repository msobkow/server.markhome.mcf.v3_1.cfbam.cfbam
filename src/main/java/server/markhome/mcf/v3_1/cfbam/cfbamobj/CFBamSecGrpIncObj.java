	...
	implements ICFBamSecGrpIncObj
{
	...
	public ICFSecSecGrpIncObj realise() {
		ICFSecSecGrpIncObj retobj = ((ICFBamSchemaObj)getSchema()).getSecGrpIncTableObj().realiseSecGrpInc(
	...
	@Override
	public void forget() {
		((ICFBamSchemaObj)getSchema()).getSecGrpIncTableObj().reallyDeepDisposeSecGrpInc( (ICFSecSecGrpIncObj)this );
	}

	@Override
	public ICFSecSecGrpIncObj read() {
		ICFSecSecGrpIncObj retobj = ((ICFBamSchemaObj)getSchema()).getSecGrpIncTableObj().readSecGrpIncByIdIdx( getPKey(), false );
		return( (ICFSecSecGrpIncObj)retobj );
	}

	@Override
	public ICFSecSecGrpIncObj read( boolean forceRead ) {
		ICFSecSecGrpIncObj retobj = ((ICFBamSchemaObj)getSchema()).getSecGrpIncTableObj().readSecGrpIncByIdIdx( getPKey(), forceRead );
		return( (ICFSecSecGrpIncObj)retobj );
	}

	@Override
	public ICFSecSecGrpIncTableObj getSecGrpIncTable() {
		return( ((ICFBamSchemaObj)getSchema()).getSecGrpIncTableObj() );
	}
	...
	public ICFSecSecGrpInc getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFSecBackingStore().getTableSecGrpInc().readDerivedByIdIdx( ((ICFBamSchemaObj)getSchema()).getAuthorization(),
						getPKey() );
				if( rec != null ) {
					copyRecToPKey();
				}
	...
	public ICFSecSecGrpIncEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFBamSchemaObj)getSchema()).getSecGrpIncTableObj().lockSecGrpInc( getPKey() );
		}
		edit = ((ICFBamSchemaObj)getSchema()).getSecGrpIncTableObj().newEditInstance( lockobj );
	...
	public ICFSecClusterObj getRequiredOwnerCluster( boolean forceRead ) {
		if( ( requiredOwnerCluster == null ) || forceRead ) {
			...
			if( ! anyMissing ) {
				requiredOwnerCluster = ((ICFBamSchemaObj)getSchema()).getClusterTableObj().readClusterByIdIdx( getSecGrpIncRec().getRequiredClusterId(), forceRead );
			}
		}
	...
	public ICFSecSecGroupObj getRequiredContainerGroup( boolean forceRead ) {
		if( ( requiredContainerGroup == null ) || forceRead ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				requiredContainerGroup = ((ICFBamSchemaObj)getSchema()).getSecGroupTableObj().readSecGroupByIdIdx( getSecGrpIncRec().getRequiredSecGroupId(), forceRead );
			}
		}
	...
	public ICFSecSecGroupObj getRequiredParentSubGroup( boolean forceRead ) {
		if( ( requiredParentSubGroup == null ) || forceRead ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				requiredParentSubGroup = ((ICFBamSchemaObj)getSchema()).getSecGroupTableObj().readSecGroupByIdIdx( getSecGrpIncRec().getRequiredIncludeGroupId(), forceRead );
			}
		}
