// Description: Java 25 edit object instance implementation for CFBam NmTokenCol.
	...
	implements ICFBamNmTokenColEditObj
{
	...
	public int getClassCode() {
		return( ((ICFBamSchemaObj)orig.getSchema()).getNmTokenColTableObj().getClassCode() );
	}
	...
	public ICFBamValue getRec() {
		if( rec == null ) {
			rec = getOrigAsNmTokenCol().getSchema().getCFBamBackingStore().getCFBamFactory().getFactoryNmTokenCol().newRec();
			rec.set( orig.getRec() );
		}
		return( rec );
	}
	...
	public ICFBamTableObj getRequiredContainerTable( boolean forceRead ) {
		if( forceRead || ( requiredContainerTable == null ) ) {
			...
			if( ! anyMissing ) {
				ICFBamTableObj obj = ((ICFBamSchemaObj)getOrigAsNmTokenCol().getSchema()).getTableTableObj().readTableByIdIdx( getNmTokenColRec().getRequiredTableId() );
	...
	public void setRequiredContainerTable( ICFBamTableObj value ) {
		...
		if( value != null ) {
			...
			getNmTokenColRec().setRequiredContainerTable(value.getTableRec());
		}
		requiredContainerTable = value;
	...
	public void copyRecToOrig() {
		ICFBamNmTokenCol origRec = getOrigAsNmTokenCol().getNmTokenColRec();
		ICFBamNmTokenCol myRec = getNmTokenColRec();
		origRec.set( myRec );
	}
