// Description: Java 25 edit object instance implementation for CFBam ClearDep.
	...
	implements ICFBamClearDepEditObj
{
	...
	public int getClassCode() {
		return( ((ICFBamSchemaObj)orig.getSchema()).getClearDepTableObj().getClassCode() );
	}
	...
	public ICFBamScope getRec() {
		if( rec == null ) {
			rec = getOrigAsClearDep().getSchema().getCFBamBackingStore().getCFBamFactory().getFactoryClearDep().newRec();
			rec.set( orig.getRec() );
		}
		return( rec );
	}
	...
	public ICFBamRelationObj getRequiredLookupRelation( boolean forceRead ) {
		if( forceRead || ( requiredLookupRelation == null ) ) {
			...
			if( ! anyMissing ) {
				ICFBamRelationObj obj = ((ICFBamSchemaObj)getOrigAsClearDep().getSchema()).getRelationTableObj().readRelationByIdIdx( getClearDepRec().getRequiredRelationId() );
	...
	public void setRequiredLookupRelation( ICFBamRelationObj value ) {
		...
		if( value != null ) {
			...
			getClearDepRec().setRequiredLookupRelation(value.getRelationRec());
		}
		else {
			requiredLookupRelation = null;
			getClearDepRec().setRequiredLookupRelation((ICFBamRelation)null);
		}
		requiredLookupRelation = value;
	}
	...
	public ICFBamSchemaDefObj getOptionalLookupDefSchema( boolean forceRead ) {
		if( forceRead || ( optionalLookupDefSchema == null ) ) {
			...
			if( ! anyMissing ) {
				ICFBamSchemaDefObj obj = ((ICFBamSchemaObj)getOrigAsClearDep().getSchema()).getSchemaDefTableObj().readSchemaDefByIdIdx( getClearDepRec().getOptionalDefSchemaId() );
	...
	public void setOptionalLookupDefSchema( ICFBamSchemaDefObj value ) {
		...
		if( value != null ) {
			...
			getClearDepRec().setOptionalLookupDefSchema(value.getSchemaDefRec());
		}
		else {
			optionalLookupDefSchema = null;
			getClearDepRec().setOptionalLookupDefSchema((ICFBamSchemaDef)null);
		}
		optionalLookupDefSchema = value;
	}
	...
	public void copyRecToOrig() {
		ICFBamClearDep origRec = getOrigAsClearDep().getClearDepRec();
		ICFBamClearDep myRec = getClearDepRec();
		origRec.set( myRec );
	}
