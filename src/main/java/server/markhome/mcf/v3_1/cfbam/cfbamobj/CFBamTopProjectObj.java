	...
	implements ICFBamTopProjectObj
{
	...
	public ICFIntTopProjectObj realise() {
		ICFIntTopProjectObj retobj = ((ICFBamSchemaObj)getSchema()).getTopProjectTableObj().realiseTopProject(
	...
	@Override
	public void forget() {
		((ICFBamSchemaObj)getSchema()).getTopProjectTableObj().reallyDeepDisposeTopProject( (ICFIntTopProjectObj)this );
	}

	@Override
	public ICFIntTopProjectObj read() {
		ICFIntTopProjectObj retobj = ((ICFBamSchemaObj)getSchema()).getTopProjectTableObj().readTopProjectByIdIdx( getPKey(), false );
		return( (ICFIntTopProjectObj)retobj );
	}

	@Override
	public ICFIntTopProjectObj read( boolean forceRead ) {
		ICFIntTopProjectObj retobj = ((ICFBamSchemaObj)getSchema()).getTopProjectTableObj().readTopProjectByIdIdx( getPKey(), forceRead );
		return( (ICFIntTopProjectObj)retobj );
	}

	@Override
	public ICFIntTopProjectTableObj getTopProjectTable() {
		return( ((ICFBamSchemaObj)getSchema()).getTopProjectTableObj() );
	}
	...
	public ICFIntTopProject getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFIntBackingStore().getTableTopProject().readDerivedByIdIdx( ((ICFBamSchemaObj)getSchema()).getAuthorization(),
						getPKey() );
				if( rec != null ) {
					copyRecToPKey();
				}
	...
	public ICFIntTopProjectEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFBamSchemaObj)getSchema()).getTopProjectTableObj().lockTopProject( getPKey() );
		}
		edit = ((ICFBamSchemaObj)getSchema()).getTopProjectTableObj().newEditInstance( lockobj );
	...
	public ICFSecTenantObj getRequiredOwnerTenant( boolean forceRead ) {
		if( ( requiredOwnerTenant == null ) || forceRead ) {
			...
			if( ! anyMissing ) {
				requiredOwnerTenant = ((ICFBamSchemaObj)getSchema()).getTenantTableObj().readTenantByIdIdx( getTopProjectRec().getRequiredTenantId(), forceRead );
			}
		}
	...
	public ICFIntTopDomainObj getRequiredContainerParentSDom( boolean forceRead ) {
		if( ( requiredContainerParentSDom == null ) || forceRead ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				requiredContainerParentSDom = ((ICFBamSchemaObj)getSchema()).getTopDomainTableObj().readTopDomainByIdIdx( getTopProjectRec().getRequiredTopDomainId(), forceRead );
			}
		}
	...
	public List<ICFIntSubProjectObj> getOptionalComponentsSubProject() {
		...
		retval = ((ICFBamSchemaObj)getSchema()).getSubProjectTableObj().readSubProjectByTopProjectIdx( getPKey(),
			false );
		return( retval );
	}
