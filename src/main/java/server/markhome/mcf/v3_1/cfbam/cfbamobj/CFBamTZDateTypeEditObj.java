// Description: Java 25 edit object instance implementation for CFBam TZDateType.
	...
	implements ICFBamTZDateTypeEditObj
{
	...
	public int getClassCode() {
		return( ((ICFBamSchemaObj)orig.getSchema()).getTZDateTypeTableObj().getClassCode() );
	}
	...
	public ICFBamValue getRec() {
		if( rec == null ) {
			rec = getOrigAsTZDateType().getSchema().getCFBamBackingStore().getCFBamFactory().getFactoryTZDateType().newRec();
			rec.set( orig.getRec() );
		}
		return( rec );
	}
	...
	public ICFBamSchemaDefObj getRequiredContainerSchemaDef( boolean forceRead ) {
		if( forceRead || ( requiredContainerSchemaDef == null ) ) {
			...
			if( ! anyMissing ) {
				ICFBamSchemaDefObj obj = ((ICFBamSchemaObj)getOrigAsTZDateType().getSchema()).getSchemaDefTableObj().readSchemaDefByIdIdx( getTZDateTypeRec().getRequiredSchemaDefId() );
	...
	public void setRequiredContainerSchemaDef( ICFBamSchemaDefObj value ) {
		...
		if( value != null ) {
			...
			getTZDateTypeRec().setRequiredContainerSchemaDef(value.getSchemaDefRec());
		}
		requiredContainerSchemaDef = value;
	...
	public void copyRecToOrig() {
		ICFBamTZDateType origRec = getOrigAsTZDateType().getTZDateTypeRec();
		ICFBamTZDateType myRec = getTZDateTypeRec();
		origRec.set( myRec );
	}
