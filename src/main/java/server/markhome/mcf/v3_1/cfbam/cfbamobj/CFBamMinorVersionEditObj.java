// Description: Java 25 edit object instance implementation for CFBam MinorVersion.
	...
	implements ICFBamMinorVersionEditObj
{
	...
	public void setRequiredId(CFLibDbKeyHash256 id) {
		if (getPKey() != id) {
			...
			requiredOwnerTenant = null;
			requiredContainerParentMajVer = null;
			optionalComponentsSchemaDef = null;
		}
	}
	...
	public ICFSecTenantObj getRequiredOwnerTenant( boolean forceRead ) {
		if( forceRead || ( requiredOwnerTenant == null ) ) {
			...
			if( ! anyMissing ) {
				ICFSecTenantObj obj = ((ICFBamSchemaObj)getOrigAsMinorVersion().getSchema()).getTenantTableObj().readTenantByIdIdx( getMinorVersionRec().getRequiredTenantId() );
	...
	public ICFIntMajorVersionObj getRequiredContainerParentMajVer( boolean forceRead ) {
		if( forceRead || ( requiredContainerParentMajVer == null ) ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				ICFIntMajorVersionObj obj = ((ICFBamSchemaObj)getOrigAsMinorVersion().getSchema()).getMajorVersionTableObj().readMajorVersionByIdIdx( getMinorVersionRec().getRequiredMajorVersionId() );
