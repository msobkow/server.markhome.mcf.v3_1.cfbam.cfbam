	...
	implements ICFBamServiceObj
{
	...
	public ICFSecServiceObj realise() {
		ICFSecServiceObj retobj = ((ICFBamSchemaObj)getSchema()).getServiceTableObj().realiseService(
	...
	@Override
	public void forget() {
		((ICFBamSchemaObj)getSchema()).getServiceTableObj().reallyDeepDisposeService( (ICFSecServiceObj)this );
	}

	@Override
	public ICFSecServiceObj read() {
		ICFSecServiceObj retobj = ((ICFBamSchemaObj)getSchema()).getServiceTableObj().readServiceByIdIdx( getPKey(), false );
		return( (ICFSecServiceObj)retobj );
	}

	@Override
	public ICFSecServiceObj read( boolean forceRead ) {
		ICFSecServiceObj retobj = ((ICFBamSchemaObj)getSchema()).getServiceTableObj().readServiceByIdIdx( getPKey(), forceRead );
		return( (ICFSecServiceObj)retobj );
	}

	@Override
	public ICFSecServiceTableObj getServiceTable() {
		return( ((ICFBamSchemaObj)getSchema()).getServiceTableObj() );
	}
	...
	public ICFSecService getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFSecBackingStore().getTableService().readDerivedByIdIdx( ((ICFBamSchemaObj)getSchema()).getAuthorization(),
						getPKey() );
				if( rec != null ) {
					copyRecToPKey();
				}
	...
	public ICFSecServiceEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFBamSchemaObj)getSchema()).getServiceTableObj().lockService( getPKey() );
		}
		edit = ((ICFBamSchemaObj)getSchema()).getServiceTableObj().newEditInstance( lockobj );
	...
	public ICFSecClusterObj getRequiredOwnerCluster( boolean forceRead ) {
		if( ( requiredOwnerCluster == null ) || forceRead ) {
			...
			if( ! anyMissing ) {
				requiredOwnerCluster = ((ICFBamSchemaObj)getSchema()).getClusterTableObj().readClusterByIdIdx( getServiceRec().getRequiredClusterId(), forceRead );
			}
		}
	...
	public ICFSecHostNodeObj getOptionalContainerHost( boolean forceRead ) {
		if( ( optionalContainerHost == null ) || forceRead ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				optionalContainerHost = ((ICFBamSchemaObj)getSchema()).getHostNodeTableObj().readHostNodeByIdIdx( getServiceRec().getRequiredHostNodeId(), forceRead );
			}
		}
	...
	public ICFSecServiceTypeObj getOptionalParentServiceType( boolean forceRead ) {
		if( ( optionalParentServiceType == null ) || forceRead ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				optionalParentServiceType = ((ICFBamSchemaObj)getSchema()).getServiceTypeTableObj().readServiceTypeByIdIdx( getServiceRec().getRequiredServiceTypeId(), forceRead );
			}
		}
