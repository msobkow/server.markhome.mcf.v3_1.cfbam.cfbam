// Description: Java 25 edit object instance implementation for CFBam Service.
	...
	implements ICFBamServiceEditObj
{
	...
	public ICFSecClusterObj getRequiredOwnerCluster( boolean forceRead ) {
		if( forceRead || ( requiredOwnerCluster == null ) ) {
			...
			if( ! anyMissing ) {
				ICFSecClusterObj obj = ((ICFBamSchemaObj)getOrigAsService().getSchema()).getClusterTableObj().readClusterByIdIdx( getServiceRec().getRequiredClusterId() );
	...
	public ICFSecHostNodeObj getOptionalContainerHost( boolean forceRead ) {
		if( forceRead || ( optionalContainerHost == null ) ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				ICFSecHostNodeObj obj = ((ICFBamSchemaObj)getOrigAsService().getSchema()).getHostNodeTableObj().readHostNodeByIdIdx( getServiceRec().getRequiredHostNodeId() );
	...
	public ICFSecServiceTypeObj getOptionalParentServiceType( boolean forceRead ) {
		if( forceRead || ( optionalParentServiceType == null ) ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				ICFSecServiceTypeObj obj = ((ICFBamSchemaObj)getOrigAsService().getSchema()).getServiceTypeTableObj().readServiceTypeByIdIdx( getServiceRec().getRequiredServiceTypeId() );
