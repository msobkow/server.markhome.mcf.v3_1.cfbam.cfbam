// Description: Java 25 edit object instance implementation for CFBam Id32Gen.
	...
	implements ICFBamId32GenEditObj
{
	...
	public int getClassCode() {
		return( ((ICFBamSchemaObj)orig.getSchema()).getId32GenTableObj().getClassCode() );
	}
	...
	public ICFBamValue getRec() {
		if( rec == null ) {
			rec = getOrigAsId32Gen().getSchema().getCFBamBackingStore().getCFBamFactory().getFactoryId32Gen().newRec();
			rec.set( orig.getRec() );
		}
		return( rec );
	}
	...
	public void setRequiredSlice( short value ) {
		if( getId32GenRec().getRequiredSlice() != value ) {
			getId32GenRec().setRequiredSlice( value );
		}
	}
	...
	public void setRequiredBlockSize( int value ) {
		if( getId32GenRec().getRequiredBlockSize() != value ) {
			getId32GenRec().setRequiredBlockSize( value );
		}
	}

	@Override
	public void copyRecToOrig() {
		ICFBamId32Gen origRec = getOrigAsId32Gen().getId32GenRec();
		ICFBamId32Gen myRec = getId32GenRec();
		origRec.set( myRec );
	}
