// Description: Java 25 edit object instance implementation for CFBam NumberDef.
	...
	implements ICFBamNumberDefEditObj
{
	...
	public int getClassCode() {
		return( ((ICFBamSchemaObj)orig.getSchema()).getNumberDefTableObj().getClassCode() );
	}
	...
	public ICFBamValue getRec() {
		if( rec == null ) {
			rec = getOrigAsNumberDef().getSchema().getCFBamBackingStore().getCFBamFactory().getFactoryNumberDef().newRec();
			rec.set( orig.getRec() );
		}
		return( rec );
	}
	...
	public void copyRecToOrig() {
		ICFBamNumberDef origRec = getOrigAsNumberDef().getNumberDefRec();
		ICFBamNumberDef myRec = getNumberDefRec();
		origRec.set( myRec );
	}
