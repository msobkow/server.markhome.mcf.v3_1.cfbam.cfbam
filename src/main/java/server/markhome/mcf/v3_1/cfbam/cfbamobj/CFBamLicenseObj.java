	...
	implements ICFBamLicenseObj
{
	...
	public ICFIntLicenseObj realise() {
		ICFIntLicenseObj retobj = ((ICFBamSchemaObj)getSchema()).getLicenseTableObj().realiseLicense(
	...
	@Override
	public void forget() {
		((ICFBamSchemaObj)getSchema()).getLicenseTableObj().reallyDeepDisposeLicense( (ICFIntLicenseObj)this );
	}

	@Override
	public ICFIntLicenseObj read() {
		ICFIntLicenseObj retobj = ((ICFBamSchemaObj)getSchema()).getLicenseTableObj().readLicenseByIdIdx( getPKey(), false );
		return( (ICFIntLicenseObj)retobj );
	}

	@Override
	public ICFIntLicenseObj read( boolean forceRead ) {
		ICFIntLicenseObj retobj = ((ICFBamSchemaObj)getSchema()).getLicenseTableObj().readLicenseByIdIdx( getPKey(), forceRead );
		return( (ICFIntLicenseObj)retobj );
	}

	@Override
	public ICFIntLicenseTableObj getLicenseTable() {
		return( ((ICFBamSchemaObj)getSchema()).getLicenseTableObj() );
	}
	...
	public ICFIntLicense getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFIntBackingStore().getTableLicense().readDerivedByIdIdx( ((ICFBamSchemaObj)getSchema()).getAuthorization(),
						getPKey() );
				if( rec != null ) {
					copyRecToPKey();
				}
	...
	public ICFIntLicenseEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFBamSchemaObj)getSchema()).getLicenseTableObj().lockLicense( getPKey() );
		}
		edit = ((ICFBamSchemaObj)getSchema()).getLicenseTableObj().newEditInstance( lockobj );
	...
	public ICFSecTenantObj getRequiredOwnerTenant( boolean forceRead ) {
		if( ( requiredOwnerTenant == null ) || forceRead ) {
			...
			if( ! anyMissing ) {
				requiredOwnerTenant = ((ICFBamSchemaObj)getSchema()).getTenantTableObj().readTenantByIdIdx( getLicenseRec().getRequiredTenantId(), forceRead );
			}
		}
	...
	public ICFIntTopDomainObj getRequiredContainerTopDomain( boolean forceRead ) {
		if( ( requiredContainerTopDomain == null ) || forceRead ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				requiredContainerTopDomain = ((ICFBamSchemaObj)getSchema()).getTopDomainTableObj().readTopDomainByIdIdx( getLicenseRec().getRequiredTopDomainId(), forceRead );
			}
		}
