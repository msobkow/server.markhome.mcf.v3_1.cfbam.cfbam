// Description: Java 25 edit object instance implementation for CFBam TimeCol.
	...
	implements ICFBamTimeColEditObj
{
	...
	public int getClassCode() {
		return( ((ICFBamSchemaObj)orig.getSchema()).getTimeColTableObj().getClassCode() );
	}
	...
	public ICFBamValue getRec() {
		if( rec == null ) {
			rec = getOrigAsTimeCol().getSchema().getCFBamBackingStore().getCFBamFactory().getFactoryTimeCol().newRec();
			rec.set( orig.getRec() );
		}
		return( rec );
	}
	...
	public ICFBamTableObj getRequiredContainerTable( boolean forceRead ) {
		if( forceRead || ( requiredContainerTable == null ) ) {
			...
			if( ! anyMissing ) {
				ICFBamTableObj obj = ((ICFBamSchemaObj)getOrigAsTimeCol().getSchema()).getTableTableObj().readTableByIdIdx( getTimeColRec().getRequiredTableId() );
	...
	public void setRequiredContainerTable( ICFBamTableObj value ) {
		...
		if( value != null ) {
			...
			getTimeColRec().setRequiredContainerTable(value.getTableRec());
		}
		requiredContainerTable = value;
	...
	public void copyRecToOrig() {
		ICFBamTimeCol origRec = getOrigAsTimeCol().getTimeColRec();
		ICFBamTimeCol myRec = getTimeColRec();
		origRec.set( myRec );
	}
