// Description: Java 25 edit object instance implementation for CFBam SchemaTweak.
	...
	implements ICFBamSchemaTweakEditObj
{
	...
	public int getClassCode() {
		return( ((ICFBamSchemaObj)orig.getSchema()).getSchemaTweakTableObj().getClassCode() );
	}
	...
	public ICFBamTweak getRec() {
		if( rec == null ) {
			rec = getOrigAsSchemaTweak().getSchema().getCFBamBackingStore().getCFBamFactory().getFactorySchemaTweak().newRec();
			rec.set( orig.getRec() );
		}
		return( rec );
	}
	...
	public ICFBamSchemaDefObj getRequiredContainerSchemaDef( boolean forceRead ) {
		if( forceRead || ( requiredContainerSchemaDef == null ) ) {
			...
			if( ! anyMissing ) {
				ICFBamSchemaDefObj obj = ((ICFBamSchemaObj)getOrigAsSchemaTweak().getSchema()).getSchemaDefTableObj().readSchemaDefByIdIdx( getSchemaTweakRec().getRequiredSchemaDefId() );
	...
	public void setRequiredContainerSchemaDef( ICFBamSchemaDefObj value ) {
		...
		if( value != null ) {
			...
			getSchemaTweakRec().setRequiredContainerSchemaDef(value.getSchemaDefRec());
		}
		requiredContainerSchemaDef = value;
	...
	public void copyRecToOrig() {
		ICFBamSchemaTweak origRec = getOrigAsSchemaTweak().getSchemaTweakRec();
		ICFBamSchemaTweak myRec = getSchemaTweakRec();
		origRec.set( myRec );
	}
