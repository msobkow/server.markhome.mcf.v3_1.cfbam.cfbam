// Description: Java 25 edit object instance implementation for CFBam TimestampType.
	...
	implements ICFBamTimestampTypeEditObj
{
	...
	public int getClassCode() {
		return( ((ICFBamSchemaObj)orig.getSchema()).getTimestampTypeTableObj().getClassCode() );
	}
	...
	public ICFBamValue getRec() {
		if( rec == null ) {
			rec = getOrigAsTimestampType().getSchema().getCFBamBackingStore().getCFBamFactory().getFactoryTimestampType().newRec();
			rec.set( orig.getRec() );
		}
		return( rec );
	}
	...
	public ICFBamSchemaDefObj getRequiredContainerSchemaDef( boolean forceRead ) {
		if( forceRead || ( requiredContainerSchemaDef == null ) ) {
			...
			if( ! anyMissing ) {
				ICFBamSchemaDefObj obj = ((ICFBamSchemaObj)getOrigAsTimestampType().getSchema()).getSchemaDefTableObj().readSchemaDefByIdIdx( getTimestampTypeRec().getRequiredSchemaDefId() );
	...
	public void setRequiredContainerSchemaDef( ICFBamSchemaDefObj value ) {
		...
		if( value != null ) {
			...
			getTimestampTypeRec().setRequiredContainerSchemaDef(value.getSchemaDefRec());
		}
		requiredContainerSchemaDef = value;
	...
	public void copyRecToOrig() {
		ICFBamTimestampType origRec = getOrigAsTimestampType().getTimestampTypeRec();
		ICFBamTimestampType myRec = getTimestampTypeRec();
		origRec.set( myRec );
	}
