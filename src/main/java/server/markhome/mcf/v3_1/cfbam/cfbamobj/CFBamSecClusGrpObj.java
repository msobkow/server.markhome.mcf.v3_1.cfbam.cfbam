...
public class CFBamSecClusGrpObj
	implements ICFBamSecClusGrpObj
{
	...
	public ICFSecSecClusGrpObj realise() {
		ICFSecSecClusGrpObj retobj = ((ICFBamSchemaObj)getSchema()).getSecClusGrpTableObj().realiseSecClusGrp(
	...
	@Override
	public void forget() {
		((ICFBamSchemaObj)getSchema()).getSecClusGrpTableObj().reallyDeepDisposeSecClusGrp( (ICFSecSecClusGrpObj)this );
	}

	@Override
	public ICFSecSecClusGrpObj read() {
		ICFSecSecClusGrpObj retobj = ((ICFBamSchemaObj)getSchema()).getSecClusGrpTableObj().readSecClusGrpByIdIdx( getPKey(), false );
		return( (ICFSecSecClusGrpObj)retobj );
	}

	@Override
	public ICFSecSecClusGrpObj read( boolean forceRead ) {
		ICFSecSecClusGrpObj retobj = ((ICFBamSchemaObj)getSchema()).getSecClusGrpTableObj().readSecClusGrpByIdIdx( getPKey(), forceRead );
		return( (ICFSecSecClusGrpObj)retobj );
	}

	@Override
	public ICFSecSecClusGrpTableObj getSecClusGrpTable() {
		return( ((ICFBamSchemaObj)getSchema()).getSecClusGrpTableObj() );
	}
	...
	public ICFSecSecClusGrp getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFSecBackingStore().getTableSecClusGrp().readDerivedByIdIdx( ((ICFBamSchemaObj)getSchema()).getAuthorization(),
						getPKey() );
				if( rec != null ) {
					copyRecToPKey();
				}
	...
	public ICFSecSecClusGrpEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFBamSchemaObj)getSchema()).getSecClusGrpTableObj().lockSecClusGrp( getPKey() );
		}
		edit = ((ICFBamSchemaObj)getSchema()).getSecClusGrpTableObj().newEditInstance( lockobj );
	...
	public ICFSecClusterObj getRequiredOwnerCluster( boolean forceRead ) {
		if( ( requiredOwnerCluster == null ) || forceRead ) {
			...
			if( ! anyMissing ) {
				requiredOwnerCluster = ((ICFBamSchemaObj)getSchema()).getClusterTableObj().readClusterByIdIdx( getSecClusGrpRec().getRequiredClusterId(), forceRead );
			}
		}
	...
	public List<ICFSecSecClusGrpMembObj> getOptionalChildrenMembByGrp() {
		...
		retval = ((ICFBamSchemaObj)getSchema()).getSecClusGrpMembTableObj().readSecClusGrpMembByClusGrpIdx( getPKey(),
			false );
		return( retval );
	}
