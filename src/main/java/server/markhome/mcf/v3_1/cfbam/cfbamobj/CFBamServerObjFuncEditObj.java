// Description: Java 25 edit object instance implementation for CFBam ServerObjFunc.
	...
	implements ICFBamServerObjFuncEditObj
{
	...
	public int getClassCode() {
		return( ((ICFBamSchemaObj)orig.getSchema()).getServerObjFuncTableObj().getClassCode() );
	}
	...
	public ICFBamScope getRec() {
		if( rec == null ) {
			rec = getOrigAsServerObjFunc().getSchema().getCFBamBackingStore().getCFBamFactory().getFactoryServerObjFunc().newRec();
			rec.set( orig.getRec() );
		}
		return( rec );
	}
	...
	public ICFBamTableObj getOptionalLookupRetTable( boolean forceRead ) {
		if( forceRead || ( optionalLookupRetTable == null ) ) {
			...
			if( ! anyMissing ) {
				ICFBamTableObj obj = ((ICFBamSchemaObj)getOrigAsServerObjFunc().getSchema()).getTableTableObj().readTableByIdIdx( getServerObjFuncRec().getOptionalRetTableId() );
	...
	public void setOptionalLookupRetTable( ICFBamTableObj value ) {
		...
		if( value != null ) {
			...
			getServerObjFuncRec().setOptionalLookupRetTable(value.getTableRec());
		}
		else {
			optionalLookupRetTable = null;
			getServerObjFuncRec().setOptionalLookupRetTable((ICFBamTable)null);
		}
		optionalLookupRetTable = value;
	}
	...
	public void copyRecToOrig() {
		ICFBamServerObjFunc origRec = getOrigAsServerObjFunc().getServerObjFuncRec();
		ICFBamServerObjFunc myRec = getServerObjFuncRec();
		origRec.set( myRec );
	}
