	...
	implements ICFBamISOCtryObj
{
	...
	public ICFSecISOCtryObj realise() {
		ICFSecISOCtryObj retobj = ((ICFBamSchemaObj)getSchema()).getISOCtryTableObj().realiseISOCtry(
	...
	@Override
	public void forget() {
		((ICFBamSchemaObj)getSchema()).getISOCtryTableObj().reallyDeepDisposeISOCtry( (ICFSecISOCtryObj)this );
	}

	@Override
	public ICFSecISOCtryObj read() {
		ICFSecISOCtryObj retobj = ((ICFBamSchemaObj)getSchema()).getISOCtryTableObj().readISOCtryByIdIdx( getPKey(), false );
		return( (ICFSecISOCtryObj)retobj );
	}

	@Override
	public ICFSecISOCtryObj read( boolean forceRead ) {
		ICFSecISOCtryObj retobj = ((ICFBamSchemaObj)getSchema()).getISOCtryTableObj().readISOCtryByIdIdx( getPKey(), forceRead );
		return( (ICFSecISOCtryObj)retobj );
	}

	@Override
	public ICFSecISOCtryTableObj getISOCtryTable() {
		return( ((ICFBamSchemaObj)getSchema()).getISOCtryTableObj() );
	}
	...
	public ICFSecISOCtry getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFSecBackingStore().getTableISOCtry().readDerivedByIdIdx( ((ICFBamSchemaObj)getSchema()).getAuthorization(),
						getPKey() );
				if( rec != null ) {
					copyRecToPKey();
				}
	...
	public ICFSecISOCtryEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFBamSchemaObj)getSchema()).getISOCtryTableObj().lockISOCtry( getPKey() );
		}
		edit = ((ICFBamSchemaObj)getSchema()).getISOCtryTableObj().newEditInstance( lockobj );
	...
	public List<ICFSecISOCtryCcyObj> getOptionalComponentsCcy() {
		...
		retval = ((ICFBamSchemaObj)getSchema()).getISOCtryCcyTableObj().readISOCtryCcyByCtryIdx( getPKey(),
			false );
		return( retval );
	}
	...
	public List<ICFSecISOCtryLangObj> getOptionalComponentsLang() {
		...
		retval = ((ICFBamSchemaObj)getSchema()).getISOCtryLangTableObj().readISOCtryLangByCtryIdx( getPKey(),
			false );
		return( retval );
	}
