// Description: Java 25 edit object instance implementation for CFBam TokenType.
	...
	implements ICFBamTokenTypeEditObj
{
	...
	public int getClassCode() {
		return( ((ICFBamSchemaObj)orig.getSchema()).getTokenTypeTableObj().getClassCode() );
	}
	...
	public ICFBamValue getRec() {
		if( rec == null ) {
			rec = getOrigAsTokenType().getSchema().getCFBamBackingStore().getCFBamFactory().getFactoryTokenType().newRec();
			rec.set( orig.getRec() );
		}
		return( rec );
	}
	...
	public ICFBamSchemaDefObj getRequiredContainerSchemaDef( boolean forceRead ) {
		if( forceRead || ( requiredContainerSchemaDef == null ) ) {
			...
			if( ! anyMissing ) {
				ICFBamSchemaDefObj obj = ((ICFBamSchemaObj)getOrigAsTokenType().getSchema()).getSchemaDefTableObj().readSchemaDefByIdIdx( getTokenTypeRec().getRequiredSchemaDefId() );
	...
	public void setRequiredContainerSchemaDef( ICFBamSchemaDefObj value ) {
		...
		if( value != null ) {
			...
			getTokenTypeRec().setRequiredContainerSchemaDef(value.getSchemaDefRec());
		}
		requiredContainerSchemaDef = value;
	...
	public void copyRecToOrig() {
		ICFBamTokenType origRec = getOrigAsTokenType().getTokenTypeRec();
		ICFBamTokenType myRec = getTokenTypeRec();
		origRec.set( myRec );
	}
