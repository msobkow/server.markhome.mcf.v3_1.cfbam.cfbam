// Description: Java 25 edit object instance implementation for CFBam SecTentGrp.
...
public class CFBamSecTentGrpEditObj
	implements ICFBamSecTentGrpEditObj
{
	...
	public CFBamSecTentGrpEditObj( ICFSecSecTentGrpObj argOrig ) {
		orig = argOrig;
		getRec();
	...
	public ICFSecSecTentGrpObj realise() {
		...
		ICFSecSecTentGrpObj retobj = getSchema().getSecTentGrpTableObj().realiseSecTentGrp( (ICFBamSecTentGrpObj)this );
		return( retobj );
	}
	...
	public ICFSecSecTentGrpObj create() {
		copyRecToOrig();
		ICFSecSecTentGrpObj retobj = ((ICFBamSchemaObj)getOrigAsSecTentGrp().getSchema()).getSecTentGrpTableObj().createSecTentGrp( getOrigAsSecTentGrp() );
	...
	public ICFSecTenantObj getRequiredOwnerTenant( boolean forceRead ) {
		if( forceRead || ( requiredOwnerTenant == null ) ) {
			...
			if( ! anyMissing ) {
				ICFSecTenantObj obj = ((ICFBamSchemaObj)getOrigAsSecTentGrp().getSchema()).getTenantTableObj().readTenantByIdIdx( getSecTentGrpRec().getRequiredTenantId() );
	...
	public ICFSecSecSysGrpObj getRequiredParentSysGrp( boolean forceRead ) {
		if( forceRead || ( requiredParentSysGrp == null ) ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				ICFSecSecSysGrpObj obj = ((ICFBamSchemaObj)getOrigAsSecTentGrp().getSchema()).getSecSysGrpTableObj().readSecSysGrpByUNameIdx( getSecTentGrpRec().getRequiredName() );
	...
	public List<ICFSecSecTentGrpMembObj> getOptionalChildrenMembByGrp() {
		...
		retval = ((ICFBamSchemaObj)getSchema()).getSecTentGrpMembTableObj().readSecTentGrpMembByTentGrpIdx( getPKey(),
			false );
		return( retval );
	}
