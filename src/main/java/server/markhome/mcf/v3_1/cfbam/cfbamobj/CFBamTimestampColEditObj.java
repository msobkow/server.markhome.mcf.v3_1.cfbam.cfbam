// Description: Java 25 edit object instance implementation for CFBam TimestampCol.
	...
	implements ICFBamTimestampColEditObj
{
	...
	public int getClassCode() {
		return( ((ICFBamSchemaObj)orig.getSchema()).getTimestampColTableObj().getClassCode() );
	}
	...
	public ICFBamValue getRec() {
		if( rec == null ) {
			rec = getOrigAsTimestampCol().getSchema().getCFBamBackingStore().getCFBamFactory().getFactoryTimestampCol().newRec();
			rec.set( orig.getRec() );
		}
		return( rec );
	}
	...
	public ICFBamTableObj getRequiredContainerTable( boolean forceRead ) {
		if( forceRead || ( requiredContainerTable == null ) ) {
			...
			if( ! anyMissing ) {
				ICFBamTableObj obj = ((ICFBamSchemaObj)getOrigAsTimestampCol().getSchema()).getTableTableObj().readTableByIdIdx( getTimestampColRec().getRequiredTableId() );
	...
	public void setRequiredContainerTable( ICFBamTableObj value ) {
		...
		if( value != null ) {
			...
			getTimestampColRec().setRequiredContainerTable(value.getTableRec());
		}
		requiredContainerTable = value;
	...
	public void copyRecToOrig() {
		ICFBamTimestampCol origRec = getOrigAsTimestampCol().getTimestampColRec();
		ICFBamTimestampCol myRec = getTimestampColRec();
		origRec.set( myRec );
	}
