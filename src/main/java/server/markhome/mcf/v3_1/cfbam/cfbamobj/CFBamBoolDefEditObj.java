// Description: Java 25 edit object instance implementation for CFBam BoolDef.
	...
	implements ICFBamBoolDefEditObj
{
	...
	public int getClassCode() {
		return( ((ICFBamSchemaObj)orig.getSchema()).getBoolDefTableObj().getClassCode() );
	}
	...
	public ICFBamValue getRec() {
		if( rec == null ) {
			rec = getOrigAsBoolDef().getSchema().getCFBamBackingStore().getCFBamFactory().getFactoryBoolDef().newRec();
			rec.set( orig.getRec() );
		}
		return( rec );
	}
	...
	public void copyRecToOrig() {
		ICFBamBoolDef origRec = getOrigAsBoolDef().getBoolDefRec();
		ICFBamBoolDef myRec = getBoolDefRec();
		origRec.set( myRec );
	}
