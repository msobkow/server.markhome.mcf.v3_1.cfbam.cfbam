	...
	implements ICFBamTSecGrpMembObj
{
	...
	public ICFSecTSecGrpMembObj realise() {
		ICFSecTSecGrpMembObj retobj = ((ICFBamSchemaObj)getSchema()).getTSecGrpMembTableObj().realiseTSecGrpMemb(
	...
	@Override
	public void forget() {
		((ICFBamSchemaObj)getSchema()).getTSecGrpMembTableObj().reallyDeepDisposeTSecGrpMemb( (ICFSecTSecGrpMembObj)this );
	}

	@Override
	public ICFSecTSecGrpMembObj read() {
		ICFSecTSecGrpMembObj retobj = ((ICFBamSchemaObj)getSchema()).getTSecGrpMembTableObj().readTSecGrpMembByIdIdx( getPKey(), false );
		return( (ICFSecTSecGrpMembObj)retobj );
	}

	@Override
	public ICFSecTSecGrpMembObj read( boolean forceRead ) {
		ICFSecTSecGrpMembObj retobj = ((ICFBamSchemaObj)getSchema()).getTSecGrpMembTableObj().readTSecGrpMembByIdIdx( getPKey(), forceRead );
		return( (ICFSecTSecGrpMembObj)retobj );
	}

	@Override
	public ICFSecTSecGrpMembTableObj getTSecGrpMembTable() {
		return( ((ICFBamSchemaObj)getSchema()).getTSecGrpMembTableObj() );
	}
	...
	public ICFSecTSecGrpMemb getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFSecBackingStore().getTableTSecGrpMemb().readDerivedByIdIdx( ((ICFBamSchemaObj)getSchema()).getAuthorization(),
						getPKey() );
				if( rec != null ) {
					copyRecToPKey();
				}
	...
	public ICFSecTSecGrpMembEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFBamSchemaObj)getSchema()).getTSecGrpMembTableObj().lockTSecGrpMemb( getPKey() );
		}
		edit = ((ICFBamSchemaObj)getSchema()).getTSecGrpMembTableObj().newEditInstance( lockobj );
	...
	public ICFSecTenantObj getRequiredOwnerTenant( boolean forceRead ) {
		if( ( requiredOwnerTenant == null ) || forceRead ) {
			...
			if( ! anyMissing ) {
				requiredOwnerTenant = ((ICFBamSchemaObj)getSchema()).getTenantTableObj().readTenantByIdIdx( getTSecGrpMembRec().getRequiredTenantId(), forceRead );
			}
		}
	...
	public ICFSecTSecGroupObj getRequiredContainerGroup( boolean forceRead ) {
		if( ( requiredContainerGroup == null ) || forceRead ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				requiredContainerGroup = ((ICFBamSchemaObj)getSchema()).getTSecGroupTableObj().readTSecGroupByIdIdx( getTSecGrpMembRec().getRequiredTSecGroupId(), forceRead );
			}
		}
	...
	public ICFSecSecUserObj getRequiredParentUser( boolean forceRead ) {
		if( ( requiredParentUser == null ) || forceRead ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				requiredParentUser = ((ICFBamSchemaObj)getSchema()).getSecUserTableObj().readSecUserByIdIdx( getTSecGrpMembRec().getRequiredSecUserId(), forceRead );
			}
		}
