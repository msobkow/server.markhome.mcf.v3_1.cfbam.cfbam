// Description: Java 25 edit object instance implementation for CFBam TopDomain.
	...
	implements ICFBamTopDomainEditObj
{
	...
	public ICFSecTenantObj getRequiredOwnerTenant( boolean forceRead ) {
		if( forceRead || ( requiredOwnerTenant == null ) ) {
			...
			if( ! anyMissing ) {
				ICFSecTenantObj obj = ((ICFBamSchemaObj)getOrigAsTopDomain().getSchema()).getTenantTableObj().readTenantByIdIdx( getTopDomainRec().getRequiredTenantId() );
	...
	public ICFIntTldObj getRequiredContainerParentTld( boolean forceRead ) {
		if( forceRead || ( requiredContainerParentTld == null ) ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				ICFIntTldObj obj = ((ICFBamSchemaObj)getOrigAsTopDomain().getSchema()).getTldTableObj().readTldByIdIdx( getTopDomainRec().getRequiredTldId() );
