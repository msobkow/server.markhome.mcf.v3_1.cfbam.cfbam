	...
	implements ICFBamSecSessionObj
{
	...
	public ICFSecSecSessionObj realise() {
		ICFSecSecSessionObj retobj = ((ICFBamSchemaObj)getSchema()).getSecSessionTableObj().realiseSecSession(
	...
	@Override
	public void forget() {
		((ICFBamSchemaObj)getSchema()).getSecSessionTableObj().reallyDeepDisposeSecSession( (ICFSecSecSessionObj)this );
	}

	@Override
	public ICFSecSecSessionObj read() {
		ICFSecSecSessionObj retobj = ((ICFBamSchemaObj)getSchema()).getSecSessionTableObj().readSecSessionByIdIdx( getPKey(), false );
		return( (ICFSecSecSessionObj)retobj );
	}

	@Override
	public ICFSecSecSessionObj read( boolean forceRead ) {
		ICFSecSecSessionObj retobj = ((ICFBamSchemaObj)getSchema()).getSecSessionTableObj().readSecSessionByIdIdx( getPKey(), forceRead );
		return( (ICFSecSecSessionObj)retobj );
	}

	@Override
	public ICFSecSecSessionTableObj getSecSessionTable() {
		return( ((ICFBamSchemaObj)getSchema()).getSecSessionTableObj() );
	}
	...
	public ICFSecSecSession getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFSecBackingStore().getTableSecSession().readDerivedByIdIdx( ((ICFBamSchemaObj)getSchema()).getAuthorization(),
						getPKey() );
				if( rec != null ) {
					copyRecToPKey();
				}
	...
	public ICFSecSecSessionEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFBamSchemaObj)getSchema()).getSecSessionTableObj().lockSecSession( getPKey() );
		}
		edit = ((ICFBamSchemaObj)getSchema()).getSecSessionTableObj().newEditInstance( lockobj );
	...
	public ICFSecSecUserObj getRequiredContainerSecUser( boolean forceRead ) {
		if( ( requiredContainerSecUser == null ) || forceRead ) {
			...
			if( ! anyMissing ) {
				requiredContainerSecUser = ((ICFBamSchemaObj)getSchema()).getSecUserTableObj().readSecUserByIdIdx( getSecSessionRec().getRequiredSecUserId(), forceRead );
			}
		}
	...
	public ICFSecSecUserObj getRequiredParentSecProxy( boolean forceRead ) {
		if( ( requiredParentSecProxy == null ) || forceRead ) {
			...
			if( ! anyMissing ) {
				requiredParentSecProxy = ((ICFBamSchemaObj)getSchema()).getSecUserTableObj().readSecUserByIdIdx( getSecSessionRec().getOptionalSecProxyId(), forceRead );
			}
		}
