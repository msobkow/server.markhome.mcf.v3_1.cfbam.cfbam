	...
	implements ICFBamMajorVersionObj
{
	...
	public ICFIntMajorVersionObj realise() {
		ICFIntMajorVersionObj retobj = ((ICFBamSchemaObj)getSchema()).getMajorVersionTableObj().realiseMajorVersion(
	...
	@Override
	public void forget() {
		((ICFBamSchemaObj)getSchema()).getMajorVersionTableObj().reallyDeepDisposeMajorVersion( (ICFIntMajorVersionObj)this );
	}

	@Override
	public ICFIntMajorVersionObj read() {
		ICFIntMajorVersionObj retobj = ((ICFBamSchemaObj)getSchema()).getMajorVersionTableObj().readMajorVersionByIdIdx( getPKey(), false );
		return( (ICFIntMajorVersionObj)retobj );
	}

	@Override
	public ICFIntMajorVersionObj read( boolean forceRead ) {
		ICFIntMajorVersionObj retobj = ((ICFBamSchemaObj)getSchema()).getMajorVersionTableObj().readMajorVersionByIdIdx( getPKey(), forceRead );
		return( (ICFIntMajorVersionObj)retobj );
	}

	@Override
	public ICFIntMajorVersionTableObj getMajorVersionTable() {
		return( ((ICFBamSchemaObj)getSchema()).getMajorVersionTableObj() );
	}
	...
	public ICFIntMajorVersion getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFIntBackingStore().getTableMajorVersion().readDerivedByIdIdx( ((ICFBamSchemaObj)getSchema()).getAuthorization(),
						getPKey() );
				if( rec != null ) {
					copyRecToPKey();
				}
	...
	public ICFIntMajorVersionEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFBamSchemaObj)getSchema()).getMajorVersionTableObj().lockMajorVersion( getPKey() );
		}
		edit = ((ICFBamSchemaObj)getSchema()).getMajorVersionTableObj().newEditInstance( lockobj );
	...
	public ICFSecTenantObj getRequiredOwnerTenant( boolean forceRead ) {
		if( ( requiredOwnerTenant == null ) || forceRead ) {
			...
			if( ! anyMissing ) {
				requiredOwnerTenant = ((ICFBamSchemaObj)getSchema()).getTenantTableObj().readTenantByIdIdx( getMajorVersionRec().getRequiredTenantId(), forceRead );
			}
		}
	...
	public ICFIntSubProjectObj getRequiredContainerParentSPrj( boolean forceRead ) {
		if( ( requiredContainerParentSPrj == null ) || forceRead ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				requiredContainerParentSPrj = ((ICFBamSchemaObj)getSchema()).getSubProjectTableObj().readSubProjectByIdIdx( getMajorVersionRec().getRequiredSubProjectId(), forceRead );
			}
		}
