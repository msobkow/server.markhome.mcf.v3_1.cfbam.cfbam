	...
	implements ICFBamISOLangObj
{
	...
	public ICFSecISOLangObj realise() {
		ICFSecISOLangObj retobj = ((ICFBamSchemaObj)getSchema()).getISOLangTableObj().realiseISOLang(
	...
	@Override
	public void forget() {
		((ICFBamSchemaObj)getSchema()).getISOLangTableObj().reallyDeepDisposeISOLang( (ICFSecISOLangObj)this );
	}

	@Override
	public ICFSecISOLangObj read() {
		ICFSecISOLangObj retobj = ((ICFBamSchemaObj)getSchema()).getISOLangTableObj().readISOLangByIdIdx( getPKey(), false );
		return( (ICFSecISOLangObj)retobj );
	}

	@Override
	public ICFSecISOLangObj read( boolean forceRead ) {
		ICFSecISOLangObj retobj = ((ICFBamSchemaObj)getSchema()).getISOLangTableObj().readISOLangByIdIdx( getPKey(), forceRead );
		return( (ICFSecISOLangObj)retobj );
	}

	@Override
	public ICFSecISOLangTableObj getISOLangTable() {
		return( ((ICFBamSchemaObj)getSchema()).getISOLangTableObj() );
	}
	...
	public ICFSecISOLang getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFSecBackingStore().getTableISOLang().readDerivedByIdIdx( ((ICFBamSchemaObj)getSchema()).getAuthorization(),
						getPKey() );
				if( rec != null ) {
					copyRecToPKey();
				}
	...
	public ICFSecISOLangEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFBamSchemaObj)getSchema()).getISOLangTableObj().lockISOLang( getPKey() );
		}
		edit = ((ICFBamSchemaObj)getSchema()).getISOLangTableObj().newEditInstance( lockobj );
	...
	public List<ICFSecISOCtryLangObj> getOptionalChildrenCtry() {
		...
		retval = ((ICFBamSchemaObj)getSchema()).getISOCtryLangTableObj().readISOCtryLangByLangIdx( getPKey(),
			false );
		return( retval );
	}
