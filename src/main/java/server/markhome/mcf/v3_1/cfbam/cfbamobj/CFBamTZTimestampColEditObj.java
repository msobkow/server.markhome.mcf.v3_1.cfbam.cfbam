// Description: Java 25 edit object instance implementation for CFBam TZTimestampCol.
	...
	implements ICFBamTZTimestampColEditObj
{
	...
	public int getClassCode() {
		return( ((ICFBamSchemaObj)orig.getSchema()).getTZTimestampColTableObj().getClassCode() );
	}
	...
	public ICFBamValue getRec() {
		if( rec == null ) {
			rec = getOrigAsTZTimestampCol().getSchema().getCFBamBackingStore().getCFBamFactory().getFactoryTZTimestampCol().newRec();
			rec.set( orig.getRec() );
		}
		return( rec );
	}
	...
	public ICFBamTableObj getRequiredContainerTable( boolean forceRead ) {
		if( forceRead || ( requiredContainerTable == null ) ) {
			...
			if( ! anyMissing ) {
				ICFBamTableObj obj = ((ICFBamSchemaObj)getOrigAsTZTimestampCol().getSchema()).getTableTableObj().readTableByIdIdx( getTZTimestampColRec().getRequiredTableId() );
	...
	public void setRequiredContainerTable( ICFBamTableObj value ) {
		if( rec == null ) {
			getTZTimestampColRec();
		}
		if( value != null ) {
			requiredContainerTable = value;
			getTZTimestampColRec().setRequiredContainerTable(value.getTableRec());
		}
		requiredContainerTable = value;
	...
	public void copyRecToOrig() {
		ICFBamTZTimestampCol origRec = getOrigAsTZTimestampCol().getTZTimestampColRec();
		ICFBamTZTimestampCol myRec = getTZTimestampColRec();
		origRec.set( myRec );
	}
