	...
	implements ICFBamISOCtryLangObj
{
	...
	public ICFSecISOCtryLangObj realise() {
		ICFSecISOCtryLangObj retobj = ((ICFBamSchemaObj)getSchema()).getISOCtryLangTableObj().realiseISOCtryLang(
	...
	@Override
	public void forget() {
		((ICFBamSchemaObj)getSchema()).getISOCtryLangTableObj().reallyDeepDisposeISOCtryLang( (ICFSecISOCtryLangObj)this );
	}

	@Override
	public ICFSecISOCtryLangObj read() {
		ICFSecISOCtryLangObj retobj = ((ICFBamSchemaObj)getSchema()).getISOCtryLangTableObj().readISOCtryLangByIdIdx( getPKey().getRequiredISOCtryId(),
	...
	public ICFSecISOCtryLangTableObj getISOCtryLangTable() {
		return( ((ICFBamSchemaObj)getSchema()).getISOCtryLangTableObj() );
	}
	...
	public ICFSecISOCtryLang getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFSecBackingStore().getTableISOCtryLang().readDerivedByIdIdx( ((ICFBamSchemaObj)getSchema()).getAuthorization(),
	...
	public ICFSecISOCtryLangEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFBamSchemaObj)getSchema()).getISOCtryLangTableObj().lockISOCtryLang( getPKey() );
		}
		edit = ((ICFBamSchemaObj)getSchema()).getISOCtryLangTableObj().newEditInstance( lockobj );
	...
	public ICFSecISOCtryObj getRequiredContainerCtry( boolean forceRead ) {
		if( ( requiredContainerCtry == null ) || forceRead ) {
			...
			if( ! anyMissing ) {
				requiredContainerCtry = ((ICFBamSchemaObj)getSchema()).getISOCtryTableObj().readISOCtryByIdIdx( getPKey().getRequiredISOCtryId(), forceRead );
			}
		}
	...
	public ICFSecISOLangObj getRequiredParentLang( boolean forceRead ) {
		if( ( requiredParentLang == null ) || forceRead ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				requiredParentLang = ((ICFBamSchemaObj)getSchema()).getISOLangTableObj().readISOLangByIdIdx( getPKey().getRequiredISOLangId(), forceRead );
			}
		}
