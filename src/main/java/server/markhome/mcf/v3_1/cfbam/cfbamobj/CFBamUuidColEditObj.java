// Description: Java 25 edit object instance implementation for CFBam UuidCol.
	...
	implements ICFBamUuidColEditObj
{
	...
	public int getClassCode() {
		return( ((ICFBamSchemaObj)orig.getSchema()).getUuidColTableObj().getClassCode() );
	}
	...
	public ICFBamValue getRec() {
		if( rec == null ) {
			rec = getOrigAsUuidCol().getSchema().getCFBamBackingStore().getCFBamFactory().getFactoryUuidCol().newRec();
			rec.set( orig.getRec() );
		}
		return( rec );
	}
	...
	public ICFBamTableObj getRequiredContainerTable( boolean forceRead ) {
		if( forceRead || ( requiredContainerTable == null ) ) {
			...
			if( ! anyMissing ) {
				ICFBamTableObj obj = ((ICFBamSchemaObj)getOrigAsUuidCol().getSchema()).getTableTableObj().readTableByIdIdx( getUuidColRec().getRequiredTableId() );
	...
	public void setRequiredContainerTable( ICFBamTableObj value ) {
		...
		if( value != null ) {
			...
			getUuidColRec().setRequiredContainerTable(value.getTableRec());
		}
		requiredContainerTable = value;
	...
	public void copyRecToOrig() {
		ICFBamUuidCol origRec = getOrigAsUuidCol().getUuidColRec();
		ICFBamUuidCol myRec = getUuidColRec();
		origRec.set( myRec );
	}
