	...
	implements ICFBamHostNodeObj
{
	...
	public ICFSecHostNodeObj realise() {
		ICFSecHostNodeObj retobj = ((ICFBamSchemaObj)getSchema()).getHostNodeTableObj().realiseHostNode(
	...
	@Override
	public void forget() {
		((ICFBamSchemaObj)getSchema()).getHostNodeTableObj().reallyDeepDisposeHostNode( (ICFSecHostNodeObj)this );
	}

	@Override
	public ICFSecHostNodeObj read() {
		ICFSecHostNodeObj retobj = ((ICFBamSchemaObj)getSchema()).getHostNodeTableObj().readHostNodeByIdIdx( getPKey(), false );
		return( (ICFSecHostNodeObj)retobj );
	}

	@Override
	public ICFSecHostNodeObj read( boolean forceRead ) {
		ICFSecHostNodeObj retobj = ((ICFBamSchemaObj)getSchema()).getHostNodeTableObj().readHostNodeByIdIdx( getPKey(), forceRead );
		return( (ICFSecHostNodeObj)retobj );
	}

	@Override
	public ICFSecHostNodeTableObj getHostNodeTable() {
		return( ((ICFBamSchemaObj)getSchema()).getHostNodeTableObj() );
	}
	...
	public ICFSecHostNode getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFSecBackingStore().getTableHostNode().readDerivedByIdIdx( ((ICFBamSchemaObj)getSchema()).getAuthorization(),
						getPKey() );
				if( rec != null ) {
					copyRecToPKey();
				}
	...
	public ICFSecHostNodeEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFBamSchemaObj)getSchema()).getHostNodeTableObj().lockHostNode( getPKey() );
		}
		edit = ((ICFBamSchemaObj)getSchema()).getHostNodeTableObj().newEditInstance( lockobj );
	...
	public ICFSecClusterObj getRequiredContainerCluster( boolean forceRead ) {
		if( ( requiredContainerCluster == null ) || forceRead ) {
			...
			if( ! anyMissing ) {
				requiredContainerCluster = ((ICFBamSchemaObj)getSchema()).getClusterTableObj().readClusterByIdIdx( getHostNodeRec().getRequiredClusterId(), forceRead );
			}
		}
	...
	public List<ICFSecServiceObj> getOptionalComponentsService() {
		...
		retval = ((ICFBamSchemaObj)getSchema()).getServiceTableObj().readServiceByHostIdx( getPKey(),
			false );
		return( retval );
	}
