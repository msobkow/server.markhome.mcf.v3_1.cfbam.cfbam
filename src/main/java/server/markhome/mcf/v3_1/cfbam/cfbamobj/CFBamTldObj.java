	...
	implements ICFBamTldObj
{
	...
	public ICFIntTldObj realise() {
		ICFIntTldObj retobj = ((ICFBamSchemaObj)getSchema()).getTldTableObj().realiseTld(
			(ICFIntTldObj)this );
	...
	@Override
	public void forget() {
		((ICFBamSchemaObj)getSchema()).getTldTableObj().reallyDeepDisposeTld( (ICFIntTldObj)this );
	}

	@Override
	public ICFIntTldObj read() {
		ICFIntTldObj retobj = ((ICFBamSchemaObj)getSchema()).getTldTableObj().readTldByIdIdx( getPKey(), false );
		return( (ICFIntTldObj)retobj );
	}

	@Override
	public ICFIntTldObj read( boolean forceRead ) {
		ICFIntTldObj retobj = ((ICFBamSchemaObj)getSchema()).getTldTableObj().readTldByIdIdx( getPKey(), forceRead );
		return( (ICFIntTldObj)retobj );
	}

	@Override
	public ICFIntTldTableObj getTldTable() {
		return( ((ICFBamSchemaObj)getSchema()).getTldTableObj() );
	}
	...
	public ICFIntTld getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFIntBackingStore().getTableTld().readDerivedByIdIdx( ((ICFBamSchemaObj)getSchema()).getAuthorization(),
						getPKey() );
				if( rec != null ) {
					copyRecToPKey();
				}
	...
	public ICFIntTldEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFBamSchemaObj)getSchema()).getTldTableObj().lockTld( getPKey() );
		}
		edit = ((ICFBamSchemaObj)getSchema()).getTldTableObj().newEditInstance( lockobj );
	...
	public ICFSecTenantObj getRequiredContainerTenant( boolean forceRead ) {
		if( ( requiredContainerTenant == null ) || forceRead ) {
			...
			if( ! anyMissing ) {
				requiredContainerTenant = ((ICFBamSchemaObj)getSchema()).getTenantTableObj().readTenantByIdIdx( getTldRec().getRequiredTenantId(), forceRead );
			}
		}
	...
	public List<ICFIntTopDomainObj> getOptionalComponentsTopDomain() {
		...
		retval = ((ICFBamSchemaObj)getSchema()).getTopDomainTableObj().readTopDomainByTldIdx( getPKey(),
			false );
		return( retval );
	}
