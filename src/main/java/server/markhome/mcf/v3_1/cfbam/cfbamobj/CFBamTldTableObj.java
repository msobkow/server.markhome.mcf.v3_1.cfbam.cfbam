	...
	implements ICFBamTldTableObj
{
	...
	private Map<$implCommaIJavaOptAtomType$, ICFIntTldObj> members;
	private Map<$implCommaIJavaOptAtomType$, ICFIntTldObj> allTld;
	private Map< ICFIntTldByTenantIdxKey,
		Map<$implCommaIJavaOptAtomType$, ICFIntTldObj > > indexByTenantIdx;
	...
	public CFBamTldTableObj() {
		schema = null;
		members = new HashMap<$implCommaIJavaOptAtomType$, ICFIntTldObj>();
		allTld = null;
	...
	public static int getBackingClassCode() {
		return( CFIntTldTableObj.getBackingClassCode() );
	}
	...
	public ICFIntTldObj realiseTld( ICFIntTldObj Obj ) {
		...
		if( members.containsKey( pkey ) && ( null != members.get( pkey ) ) ) {
			...
			if( indexByTenantIdx != null ) {
				...
				Map<$implCommaIJavaOptAtomType$, ICFIntTldObj > mapTenantIdx = indexByTenantIdx.get( keyTenantIdx );
	...
	@Override
	public ICFIntTldObj readTld( $implCommaIJavaOptAtomType$ pkey ) {
		return( readTld( pkey, false ) );
	}

	@Override
	public ICFIntTldObj readTld( $implCommaIJavaOptAtomType$ pkey, boolean forceRead ) {
	...
	@Override
	public ICFIntTldObj readCachedTld( $implCommaIJavaOptAtomType$ pkey ) {
		ICFIntTldObj obj = null;
	...
	@Override
	public void deepDisposeTld( $implCommaIJavaOptAtomType$ pkey ) {
	...
	@Override
	public ICFIntTldObj lockTld( $implCommaIJavaOptAtomType$ pkey ) {
	...
	public List<ICFIntTldObj> readAllTld( boolean forceRead ) {
		...
		if( ( allTld == null ) || forceRead ) {
			Map<$implCommaIJavaOptAtomType$, ICFIntTldObj> map = new HashMap<$implCommaIJavaOptAtomType$,ICFIntTldObj>();
			allTld = map;
		...
		boolean forceRead )
	{
		...
		Map<$implCommaIJavaOptAtomType$, ICFIntTldObj> dict;
		if( indexByTenantIdx == null ) {
			indexByTenantIdx = new HashMap< ICFIntTldByTenantIdxKey,
				Map< $implCommaIJavaOptAtomType$, ICFIntTldObj > >();
		}
		if( ( ! forceRead ) && indexByTenantIdx.containsKey( key ) ) {
			dict = indexByTenantIdx.get( key );
		}
		else {
			dict = new HashMap<$implCommaIJavaOptAtomType$, ICFIntTldObj>();
			ICFIntTldObj obj;
	...
	public void deleteTldByTenantIdx( ICFLibKeyHash256 TenantId )
	{
		...
		if( indexByTenantIdx.containsKey( key ) ) {
			Map<$implCommaIJavaOptAtomType$, ICFIntTldObj> dict = indexByTenantIdx.get( key );
