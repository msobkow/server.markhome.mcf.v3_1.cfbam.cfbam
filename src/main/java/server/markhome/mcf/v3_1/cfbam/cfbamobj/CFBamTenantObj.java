	...
	implements ICFBamTenantObj
{
	...
	public ICFSecTenantObj realise() {
		ICFSecTenantObj retobj = ((ICFBamSchemaObj)getSchema()).getTenantTableObj().realiseTenant(
	...
	@Override
	public void forget() {
		((ICFBamSchemaObj)getSchema()).getTenantTableObj().reallyDeepDisposeTenant( (ICFSecTenantObj)this );
	}

	@Override
	public ICFSecTenantObj read() {
		ICFSecTenantObj retobj = ((ICFBamSchemaObj)getSchema()).getTenantTableObj().readTenantByIdIdx( getPKey(), false );
		return( (ICFSecTenantObj)retobj );
	}

	@Override
	public ICFSecTenantObj read( boolean forceRead ) {
		ICFSecTenantObj retobj = ((ICFBamSchemaObj)getSchema()).getTenantTableObj().readTenantByIdIdx( getPKey(), forceRead );
		return( (ICFSecTenantObj)retobj );
	}

	@Override
	public ICFSecTenantTableObj getTenantTable() {
		return( ((ICFBamSchemaObj)getSchema()).getTenantTableObj() );
	}
	...
	public ICFSecTenant getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFSecBackingStore().getTableTenant().readDerivedByIdIdx( ((ICFBamSchemaObj)getSchema()).getAuthorization(),
						getPKey() );
				if( rec != null ) {
					copyRecToPKey();
				}
	...
	public ICFSecTenantEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFBamSchemaObj)getSchema()).getTenantTableObj().lockTenant( getPKey() );
		}
		edit = ((ICFBamSchemaObj)getSchema()).getTenantTableObj().newEditInstance( lockobj );
	...
	public ICFSecClusterObj getRequiredContainerCluster( boolean forceRead ) {
		if( ( requiredContainerCluster == null ) || forceRead ) {
			...
			if( ! anyMissing ) {
				requiredContainerCluster = ((ICFBamSchemaObj)getSchema()).getClusterTableObj().readClusterByIdIdx( getTenantRec().getRequiredClusterId(), forceRead );
			}
		}
	...
	public List<ICFIntTldObj> getOptionalComponentsTld() {
		...
		retval = ((ICFBamSchemaObj)getSchema()).getTldTableObj().readTldByTenantIdx( getPKey(),
			false );
		return( retval );
	}
	...
	public List<ICFBamSchemaDefObj> getOptionalComponentsSchema() {
		...
		retval = ((ICFBamSchemaObj)getSchema()).getSchemaDefTableObj().readSchemaDefByCTenantIdx( getPKey(),
			false );
		return( retval );
	}
