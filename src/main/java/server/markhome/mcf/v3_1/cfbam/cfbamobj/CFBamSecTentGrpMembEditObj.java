// Description: Java 25 edit object instance implementation for CFBam SecTentGrpMemb.
...
public class CFBamSecTentGrpMembEditObj
	implements ICFBamSecTentGrpMembEditObj
{
	...
	public CFBamSecTentGrpMembEditObj( ICFSecSecTentGrpMembObj argOrig ) {
		orig = argOrig;
		getRec();
	...
	public ICFSecSecTentGrpMembObj realise() {
		...
		ICFSecSecTentGrpMembObj retobj = getSchema().getSecTentGrpMembTableObj().realiseSecTentGrpMemb( (ICFBamSecTentGrpMembObj)this );
		return( retobj );
	}
	...
	public ICFSecSecTentGrpMembObj create() {
		copyRecToOrig();
		ICFSecSecTentGrpMembObj retobj = ((ICFBamSchemaObj)getOrigAsSecTentGrpMemb().getSchema()).getSecTentGrpMembTableObj().createSecTentGrpMemb( getOrigAsSecTentGrpMemb() );
	...
	public ICFSecSecTentGrpObj getRequiredContainerGroup( boolean forceRead ) {
		if( forceRead || ( requiredContainerGroup == null ) ) {
			...
			if( ! anyMissing ) {
				ICFSecSecTentGrpObj obj = ((ICFBamSchemaObj)getOrigAsSecTentGrpMemb().getSchema()).getSecTentGrpTableObj().readSecTentGrpByIdIdx( getPKey().getRequiredSecTentGrpId() );
	...
	public ICFSecSecUserObj getRequiredParentUser( boolean forceRead ) {
		if( forceRead || ( requiredParentUser == null ) ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				ICFSecSecUserObj obj = ((ICFBamSchemaObj)getOrigAsSecTentGrpMemb().getSchema()).getSecUserTableObj().readSecUserByULoginIdx( getPKey().getRequiredLoginId() );
