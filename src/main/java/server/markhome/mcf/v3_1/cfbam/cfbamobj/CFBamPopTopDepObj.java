// Description: Java 25 base object instance implementation for PopTopDep
	...
	implements ICFBamPopTopDepObj
{
	...
	public int getClassCode() {
		return( ((ICFBamSchemaObj)schema).getPopTopDepTableObj().getClassCode() );
	}
	...
	public ICFBamScopeObj realise() {
		ICFBamPopTopDepObj retobj = ((ICFBamSchemaObj)getSchema()).getPopTopDepTableObj().realisePopTopDep(
	...
	@Override
	public void forget() {
		((ICFBamSchemaObj)getSchema()).getPopTopDepTableObj().reallyDeepDisposePopTopDep( (ICFBamPopTopDepObj)this );
	}

	@Override
	public ICFBamScopeObj read() {
		ICFBamPopTopDepObj retobj = ((ICFBamSchemaObj)getSchema()).getPopTopDepTableObj().readPopTopDepByIdIdx( getPKey(), false );
		return( (ICFBamScopeObj)retobj );
	}

	@Override
	public ICFBamScopeObj read( boolean forceRead ) {
		ICFBamPopTopDepObj retobj = ((ICFBamSchemaObj)getSchema()).getPopTopDepTableObj().readPopTopDepByIdIdx( getPKey(), forceRead );
		return( (ICFBamScopeObj)retobj );
	}

	@Override
	public ICFBamPopTopDepTableObj getPopTopDepTable() {
		return( ((ICFBamSchemaObj)getSchema()).getPopTopDepTableObj() );
	}
	...
	public ICFBamScope getRec() {
		if( rec == null ) {
			if( isNew ) {
				rec = getSchema().getCFBamBackingStore().getCFBamFactory().getFactoryPopTopDep().newRec();
			}
			else {
				// Read the data rec via the backing store
				rec = getSchema().getCFBamBackingStore().getTablePopTopDep().readDerivedByIdIdx( ((ICFBamSchemaObj)getSchema()).getAuthorization(),
						getPKey() );
				if( rec != null ) {
					copyRecToPKey();
				}
	...
	public void setRec( ICFBamScope value ) {
		if( ! ( ( value == null ) || ! ( value instanceof ICFBamPopTopDep ) ) ) {
	...
	public ICFBamScopeEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFBamSchemaObj)getSchema()).getPopTopDepTableObj().lockPopTopDep( getPKey() );
		}
		edit = ((ICFBamSchemaObj)getSchema()).getPopTopDepTableObj().newEditInstance( lockobj );
	...
	public ICFBamRelationObj getRequiredContainerContRelation( boolean forceRead ) {
		if( ( requiredContainerContRelation == null ) || forceRead ) {
			...
			if( ! anyMissing ) {
				requiredContainerContRelation = ((ICFBamSchemaObj)getSchema()).getRelationTableObj().readRelationByIdIdx( getPopTopDepRec().getRequiredContRelationId(), forceRead );
			}
		}
