	...
	implements ICFBamSecGroupObj
{
	...
	public ICFSecSecGroupObj realise() {
		ICFSecSecGroupObj retobj = ((ICFBamSchemaObj)getSchema()).getSecGroupTableObj().realiseSecGroup(
	...
	@Override
	public void forget() {
		((ICFBamSchemaObj)getSchema()).getSecGroupTableObj().reallyDeepDisposeSecGroup( (ICFSecSecGroupObj)this );
	}

	@Override
	public ICFSecSecGroupObj read() {
		ICFSecSecGroupObj retobj = ((ICFBamSchemaObj)getSchema()).getSecGroupTableObj().readSecGroupByIdIdx( getPKey(), false );
		return( (ICFSecSecGroupObj)retobj );
	}

	@Override
	public ICFSecSecGroupObj read( boolean forceRead ) {
		ICFSecSecGroupObj retobj = ((ICFBamSchemaObj)getSchema()).getSecGroupTableObj().readSecGroupByIdIdx( getPKey(), forceRead );
		return( (ICFSecSecGroupObj)retobj );
	}

	@Override
	public ICFSecSecGroupTableObj getSecGroupTable() {
		return( ((ICFBamSchemaObj)getSchema()).getSecGroupTableObj() );
	}
	...
	public ICFSecSecGroup getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFSecBackingStore().getTableSecGroup().readDerivedByIdIdx( ((ICFBamSchemaObj)getSchema()).getAuthorization(),
						getPKey() );
				if( rec != null ) {
					copyRecToPKey();
				}
	...
	public ICFSecSecGroupEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFBamSchemaObj)getSchema()).getSecGroupTableObj().lockSecGroup( getPKey() );
		}
		edit = ((ICFBamSchemaObj)getSchema()).getSecGroupTableObj().newEditInstance( lockobj );
	...
	public ICFSecClusterObj getRequiredContainerCluster( boolean forceRead ) {
		if( ( requiredContainerCluster == null ) || forceRead ) {
			...
			if( ! anyMissing ) {
				requiredContainerCluster = ((ICFBamSchemaObj)getSchema()).getClusterTableObj().readClusterByIdIdx( getSecGroupRec().getRequiredClusterId(), forceRead );
			}
		}
	...
	public List<ICFSecSecGrpIncObj> getOptionalComponentsInclude() {
		...
		retval = ((ICFBamSchemaObj)getSchema()).getSecGrpIncTableObj().readSecGrpIncByGroupIdx( getPKey(),
			false );
		return( retval );
	}
	...
	public List<ICFSecSecGrpMembObj> getOptionalComponentsMember() {
		...
		retval = ((ICFBamSchemaObj)getSchema()).getSecGrpMembTableObj().readSecGrpMembByGroupIdx( getPKey(),
			false );
		return( retval );
	}
	...
	public List<ICFSecSecGrpIncObj> getRequiredChildrenIncByGroup() {
		List<ICFSecSecGrpIncObj> retval;
		retval = ((ICFBamSchemaObj)getSchema()).getSecGrpIncTableObj().readSecGrpIncByIncludeIdx( getPKey(),
			false );
		return( retval );
	}
