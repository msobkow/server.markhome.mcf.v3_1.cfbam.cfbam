// Description: Java 25 edit object instance implementation for CFBam TZDateCol.
	...
	implements ICFBamTZDateColEditObj
{
	...
	public int getClassCode() {
		return( ((ICFBamSchemaObj)orig.getSchema()).getTZDateColTableObj().getClassCode() );
	}
	...
	public ICFBamValue getRec() {
		if( rec == null ) {
			rec = getOrigAsTZDateCol().getSchema().getCFBamBackingStore().getCFBamFactory().getFactoryTZDateCol().newRec();
			rec.set( orig.getRec() );
		}
		return( rec );
	}
	...
	public ICFBamTableObj getRequiredContainerTable( boolean forceRead ) {
		if( forceRead || ( requiredContainerTable == null ) ) {
			...
			if( ! anyMissing ) {
				ICFBamTableObj obj = ((ICFBamSchemaObj)getOrigAsTZDateCol().getSchema()).getTableTableObj().readTableByIdIdx( getTZDateColRec().getRequiredTableId() );
	...
	public void setRequiredContainerTable( ICFBamTableObj value ) {
		...
		if( value != null ) {
			...
			getTZDateColRec().setRequiredContainerTable(value.getTableRec());
		}
		requiredContainerTable = value;
	...
	public void copyRecToOrig() {
		ICFBamTZDateCol origRec = getOrigAsTZDateCol().getTZDateColRec();
		ICFBamTZDateCol myRec = getTZDateColRec();
		origRec.set( myRec );
	}
