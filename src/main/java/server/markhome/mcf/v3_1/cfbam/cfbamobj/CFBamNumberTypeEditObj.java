// Description: Java 25 edit object instance implementation for CFBam NumberType.
	...
	implements ICFBamNumberTypeEditObj
{
	...
	public int getClassCode() {
		return( ((ICFBamSchemaObj)orig.getSchema()).getNumberTypeTableObj().getClassCode() );
	}
	...
	public ICFBamValue getRec() {
		if( rec == null ) {
			rec = getOrigAsNumberType().getSchema().getCFBamBackingStore().getCFBamFactory().getFactoryNumberType().newRec();
			rec.set( orig.getRec() );
		}
		return( rec );
	}
	...
	public ICFBamSchemaDefObj getRequiredContainerSchemaDef( boolean forceRead ) {
		if( forceRead || ( requiredContainerSchemaDef == null ) ) {
			...
			if( ! anyMissing ) {
				ICFBamSchemaDefObj obj = ((ICFBamSchemaObj)getOrigAsNumberType().getSchema()).getSchemaDefTableObj().readSchemaDefByIdIdx( getNumberTypeRec().getRequiredSchemaDefId() );
	...
	public void setRequiredContainerSchemaDef( ICFBamSchemaDefObj value ) {
		...
		if( value != null ) {
			...
			getNumberTypeRec().setRequiredContainerSchemaDef(value.getSchemaDefRec());
		}
		requiredContainerSchemaDef = value;
	...
	public void copyRecToOrig() {
		ICFBamNumberType origRec = getOrigAsNumberType().getNumberTypeRec();
		ICFBamNumberType myRec = getNumberTypeRec();
		origRec.set( myRec );
	}
