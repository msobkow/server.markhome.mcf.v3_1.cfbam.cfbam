// Description: Java 25 edit object instance implementation for CFBam TimeType.
	...
	implements ICFBamTimeTypeEditObj
{
	...
	public int getClassCode() {
		return( ((ICFBamSchemaObj)orig.getSchema()).getTimeTypeTableObj().getClassCode() );
	}
	...
	public ICFBamValue getRec() {
		if( rec == null ) {
			rec = getOrigAsTimeType().getSchema().getCFBamBackingStore().getCFBamFactory().getFactoryTimeType().newRec();
			rec.set( orig.getRec() );
		}
		return( rec );
	}
	...
	public ICFBamSchemaDefObj getRequiredContainerSchemaDef( boolean forceRead ) {
		if( forceRead || ( requiredContainerSchemaDef == null ) ) {
			...
			if( ! anyMissing ) {
				ICFBamSchemaDefObj obj = ((ICFBamSchemaObj)getOrigAsTimeType().getSchema()).getSchemaDefTableObj().readSchemaDefByIdIdx( getTimeTypeRec().getRequiredSchemaDefId() );
	...
	public void setRequiredContainerSchemaDef( ICFBamSchemaDefObj value ) {
		...
		if( value != null ) {
			...
			getTimeTypeRec().setRequiredContainerSchemaDef(value.getSchemaDefRec());
		}
		requiredContainerSchemaDef = value;
	...
	public void copyRecToOrig() {
		ICFBamTimeType origRec = getOrigAsTimeType().getTimeTypeRec();
		ICFBamTimeType myRec = getTimeTypeRec();
		origRec.set( myRec );
	}
