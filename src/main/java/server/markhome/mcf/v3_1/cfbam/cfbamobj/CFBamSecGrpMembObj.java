	...
	implements ICFBamSecGrpMembObj
{
	...
	public ICFSecSecGrpMembObj realise() {
		ICFSecSecGrpMembObj retobj = ((ICFBamSchemaObj)getSchema()).getSecGrpMembTableObj().realiseSecGrpMemb(
	...
	@Override
	public void forget() {
		((ICFBamSchemaObj)getSchema()).getSecGrpMembTableObj().reallyDeepDisposeSecGrpMemb( (ICFSecSecGrpMembObj)this );
	}

	@Override
	public ICFSecSecGrpMembObj read() {
		ICFSecSecGrpMembObj retobj = ((ICFBamSchemaObj)getSchema()).getSecGrpMembTableObj().readSecGrpMembByIdIdx( getPKey(), false );
		return( (ICFSecSecGrpMembObj)retobj );
	}

	@Override
	public ICFSecSecGrpMembObj read( boolean forceRead ) {
		ICFSecSecGrpMembObj retobj = ((ICFBamSchemaObj)getSchema()).getSecGrpMembTableObj().readSecGrpMembByIdIdx( getPKey(), forceRead );
		return( (ICFSecSecGrpMembObj)retobj );
	}

	@Override
	public ICFSecSecGrpMembTableObj getSecGrpMembTable() {
		return( ((ICFBamSchemaObj)getSchema()).getSecGrpMembTableObj() );
	}
	...
	public ICFSecSecGrpMemb getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFSecBackingStore().getTableSecGrpMemb().readDerivedByIdIdx( ((ICFBamSchemaObj)getSchema()).getAuthorization(),
						getPKey() );
				if( rec != null ) {
					copyRecToPKey();
				}
	...
	public ICFSecSecGrpMembEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFBamSchemaObj)getSchema()).getSecGrpMembTableObj().lockSecGrpMemb( getPKey() );
		}
		edit = ((ICFBamSchemaObj)getSchema()).getSecGrpMembTableObj().newEditInstance( lockobj );
	...
	public ICFSecClusterObj getRequiredOwnerCluster( boolean forceRead ) {
		if( ( requiredOwnerCluster == null ) || forceRead ) {
			...
			if( ! anyMissing ) {
				requiredOwnerCluster = ((ICFBamSchemaObj)getSchema()).getClusterTableObj().readClusterByIdIdx( getSecGrpMembRec().getRequiredClusterId(), forceRead );
			}
		}
	...
	public ICFSecSecGroupObj getRequiredContainerGroup( boolean forceRead ) {
		if( ( requiredContainerGroup == null ) || forceRead ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				requiredContainerGroup = ((ICFBamSchemaObj)getSchema()).getSecGroupTableObj().readSecGroupByIdIdx( getSecGrpMembRec().getRequiredSecGroupId(), forceRead );
			}
		}
	...
	public ICFSecSecUserObj getRequiredParentUser( boolean forceRead ) {
		if( ( requiredParentUser == null ) || forceRead ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				requiredParentUser = ((ICFBamSchemaObj)getSchema()).getSecUserTableObj().readSecUserByIdIdx( getSecGrpMembRec().getRequiredSecUserId(), forceRead );
			}
		}
