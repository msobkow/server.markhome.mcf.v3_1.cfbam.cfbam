// Description: Java 25 edit object instance implementation for CFBam TimeDef.
	...
	implements ICFBamTimeDefEditObj
{
	...
	public int getClassCode() {
		return( ((ICFBamSchemaObj)orig.getSchema()).getTimeDefTableObj().getClassCode() );
	}
	...
	public ICFBamValue getRec() {
		if( rec == null ) {
			rec = getOrigAsTimeDef().getSchema().getCFBamBackingStore().getCFBamFactory().getFactoryTimeDef().newRec();
			rec.set( orig.getRec() );
		}
		return( rec );
	}
	...
	public void setOptionalDummy( String value ) {
		if( getTimeDefRec().getOptionalDummy() != value ) {
			getTimeDefRec().setOptionalDummy( value );
		}
	}

	@Override
	public void copyRecToOrig() {
		ICFBamTimeDef origRec = getOrigAsTimeDef().getTimeDefRec();
		ICFBamTimeDef myRec = getTimeDefRec();
		origRec.set( myRec );
	}
