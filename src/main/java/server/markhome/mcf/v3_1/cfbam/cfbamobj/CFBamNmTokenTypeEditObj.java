// Description: Java 25 edit object instance implementation for CFBam NmTokenType.
	...
	implements ICFBamNmTokenTypeEditObj
{
	...
	public int getClassCode() {
		return( ((ICFBamSchemaObj)orig.getSchema()).getNmTokenTypeTableObj().getClassCode() );
	}
	...
	public ICFBamValue getRec() {
		if( rec == null ) {
			rec = getOrigAsNmTokenType().getSchema().getCFBamBackingStore().getCFBamFactory().getFactoryNmTokenType().newRec();
			rec.set( orig.getRec() );
		}
		return( rec );
	}
	...
	public ICFBamSchemaDefObj getRequiredContainerSchemaDef( boolean forceRead ) {
		if( forceRead || ( requiredContainerSchemaDef == null ) ) {
			...
			if( ! anyMissing ) {
				ICFBamSchemaDefObj obj = ((ICFBamSchemaObj)getOrigAsNmTokenType().getSchema()).getSchemaDefTableObj().readSchemaDefByIdIdx( getNmTokenTypeRec().getRequiredSchemaDefId() );
	...
	public void setRequiredContainerSchemaDef( ICFBamSchemaDefObj value ) {
		...
		if( value != null ) {
			...
			getNmTokenTypeRec().setRequiredContainerSchemaDef(value.getSchemaDefRec());
		}
		requiredContainerSchemaDef = value;
	...
	public void copyRecToOrig() {
		ICFBamNmTokenType origRec = getOrigAsNmTokenType().getNmTokenTypeRec();
		ICFBamNmTokenType myRec = getNmTokenTypeRec();
		origRec.set( myRec );
	}
