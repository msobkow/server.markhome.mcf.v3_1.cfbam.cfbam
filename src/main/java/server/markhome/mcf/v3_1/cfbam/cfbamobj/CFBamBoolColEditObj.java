// Description: Java 25 edit object instance implementation for CFBam BoolCol.
	...
	implements ICFBamBoolColEditObj
{
	...
	public int getClassCode() {
		return( ((ICFBamSchemaObj)orig.getSchema()).getBoolColTableObj().getClassCode() );
	}
	...
	public ICFBamValue getRec() {
		if( rec == null ) {
			rec = getOrigAsBoolCol().getSchema().getCFBamBackingStore().getCFBamFactory().getFactoryBoolCol().newRec();
			rec.set( orig.getRec() );
		}
		return( rec );
	}
	...
	public ICFBamTableObj getRequiredContainerTable( boolean forceRead ) {
		if( forceRead || ( requiredContainerTable == null ) ) {
			...
			if( ! anyMissing ) {
				ICFBamTableObj obj = ((ICFBamSchemaObj)getOrigAsBoolCol().getSchema()).getTableTableObj().readTableByIdIdx( getBoolColRec().getRequiredTableId() );
	...
	public void setRequiredContainerTable( ICFBamTableObj value ) {
		...
		if( value != null ) {
			...
			getBoolColRec().setRequiredContainerTable(value.getTableRec());
		}
		requiredContainerTable = value;
	...
	public void copyRecToOrig() {
		ICFBamBoolCol origRec = getOrigAsBoolCol().getBoolColRec();
		ICFBamBoolCol myRec = getBoolColRec();
		origRec.set( myRec );
	}
