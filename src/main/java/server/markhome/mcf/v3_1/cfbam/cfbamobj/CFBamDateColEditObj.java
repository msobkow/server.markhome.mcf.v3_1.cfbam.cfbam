// Description: Java 25 edit object instance implementation for CFBam DateCol.
	...
	implements ICFBamDateColEditObj
{
	...
	public int getClassCode() {
		return( ((ICFBamSchemaObj)orig.getSchema()).getDateColTableObj().getClassCode() );
	}
	...
	public ICFBamValue getRec() {
		if( rec == null ) {
			rec = getOrigAsDateCol().getSchema().getCFBamBackingStore().getCFBamFactory().getFactoryDateCol().newRec();
			rec.set( orig.getRec() );
		}
		return( rec );
	}
	...
	public ICFBamTableObj getRequiredContainerTable( boolean forceRead ) {
		if( forceRead || ( requiredContainerTable == null ) ) {
			...
			if( ! anyMissing ) {
				ICFBamTableObj obj = ((ICFBamSchemaObj)getOrigAsDateCol().getSchema()).getTableTableObj().readTableByIdIdx( getDateColRec().getRequiredTableId() );
	...
	public void setRequiredContainerTable( ICFBamTableObj value ) {
		...
		if( value != null ) {
			...
			getDateColRec().setRequiredContainerTable(value.getTableRec());
		}
		requiredContainerTable = value;
	...
	public void copyRecToOrig() {
		ICFBamDateCol origRec = getOrigAsDateCol().getDateColRec();
		ICFBamDateCol myRec = getDateColRec();
		origRec.set( myRec );
	}
