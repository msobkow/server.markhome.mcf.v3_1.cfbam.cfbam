// Description: Java 25 edit object instance implementation for CFBam BlobCol.
	...
	implements ICFBamBlobColEditObj
{
	...
	public int getClassCode() {
		return( ((ICFBamSchemaObj)orig.getSchema()).getBlobColTableObj().getClassCode() );
	}
	...
	public ICFBamValue getRec() {
		if( rec == null ) {
			rec = getOrigAsBlobCol().getSchema().getCFBamBackingStore().getCFBamFactory().getFactoryBlobCol().newRec();
			rec.set( orig.getRec() );
		}
		return( rec );
	}
	...
	public ICFBamTableObj getRequiredContainerTable( boolean forceRead ) {
		if( forceRead || ( requiredContainerTable == null ) ) {
			...
			if( ! anyMissing ) {
				ICFBamTableObj obj = ((ICFBamSchemaObj)getOrigAsBlobCol().getSchema()).getTableTableObj().readTableByIdIdx( getBlobColRec().getRequiredTableId() );
	...
	public void setRequiredContainerTable( ICFBamTableObj value ) {
		...
		if( value != null ) {
			...
			getBlobColRec().setRequiredContainerTable(value.getTableRec());
		}
		requiredContainerTable = value;
	...
	public void copyRecToOrig() {
		ICFBamBlobCol origRec = getOrigAsBlobCol().getBlobColRec();
		ICFBamBlobCol myRec = getBlobColRec();
		origRec.set( myRec );
	}
