	...
	implements ICFBamTSecGroupObj
{
	...
	public ICFSecTSecGroupObj realise() {
		ICFSecTSecGroupObj retobj = ((ICFBamSchemaObj)getSchema()).getTSecGroupTableObj().realiseTSecGroup(
	...
	@Override
	public void forget() {
		((ICFBamSchemaObj)getSchema()).getTSecGroupTableObj().reallyDeepDisposeTSecGroup( (ICFSecTSecGroupObj)this );
	}

	@Override
	public ICFSecTSecGroupObj read() {
		ICFSecTSecGroupObj retobj = ((ICFBamSchemaObj)getSchema()).getTSecGroupTableObj().readTSecGroupByIdIdx( getPKey(), false );
		return( (ICFSecTSecGroupObj)retobj );
	}

	@Override
	public ICFSecTSecGroupObj read( boolean forceRead ) {
		ICFSecTSecGroupObj retobj = ((ICFBamSchemaObj)getSchema()).getTSecGroupTableObj().readTSecGroupByIdIdx( getPKey(), forceRead );
		return( (ICFSecTSecGroupObj)retobj );
	}

	@Override
	public ICFSecTSecGroupTableObj getTSecGroupTable() {
		return( ((ICFBamSchemaObj)getSchema()).getTSecGroupTableObj() );
	}
	...
	public ICFSecTSecGroup getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFSecBackingStore().getTableTSecGroup().readDerivedByIdIdx( ((ICFBamSchemaObj)getSchema()).getAuthorization(),
						getPKey() );
				if( rec != null ) {
					copyRecToPKey();
				}
	...
	public ICFSecTSecGroupEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFBamSchemaObj)getSchema()).getTSecGroupTableObj().lockTSecGroup( getPKey() );
		}
		edit = ((ICFBamSchemaObj)getSchema()).getTSecGroupTableObj().newEditInstance( lockobj );
	...
	public ICFSecTenantObj getRequiredContainerTenant( boolean forceRead ) {
		if( ( requiredContainerTenant == null ) || forceRead ) {
			...
			if( ! anyMissing ) {
				requiredContainerTenant = ((ICFBamSchemaObj)getSchema()).getTenantTableObj().readTenantByIdIdx( getTSecGroupRec().getRequiredTenantId(), forceRead );
			}
		}
	...
	public List<ICFSecTSecGrpIncObj> getOptionalComponentsInclude() {
		...
		retval = ((ICFBamSchemaObj)getSchema()).getTSecGrpIncTableObj().readTSecGrpIncByGroupIdx( getPKey(),
			false );
		return( retval );
	}
	...
	public List<ICFSecTSecGrpMembObj> getOptionalComponentsMember() {
		...
		retval = ((ICFBamSchemaObj)getSchema()).getTSecGrpMembTableObj().readTSecGrpMembByGroupIdx( getPKey(),
			false );
		return( retval );
	}
	...
	public List<ICFSecTSecGrpIncObj> getRequiredChildrenIncByGroup() {
		List<ICFSecTSecGrpIncObj> retval;
		retval = ((ICFBamSchemaObj)getSchema()).getTSecGrpIncTableObj().readTSecGrpIncByIncludeIdx( getPKey(),
			false );
		return( retval );
	}
