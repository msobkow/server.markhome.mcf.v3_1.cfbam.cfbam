	...
	implements ICFBamTSecGrpIncObj
{
	...
	public ICFSecTSecGrpIncObj realise() {
		ICFSecTSecGrpIncObj retobj = ((ICFBamSchemaObj)getSchema()).getTSecGrpIncTableObj().realiseTSecGrpInc(
	...
	@Override
	public void forget() {
		((ICFBamSchemaObj)getSchema()).getTSecGrpIncTableObj().reallyDeepDisposeTSecGrpInc( (ICFSecTSecGrpIncObj)this );
	}

	@Override
	public ICFSecTSecGrpIncObj read() {
		ICFSecTSecGrpIncObj retobj = ((ICFBamSchemaObj)getSchema()).getTSecGrpIncTableObj().readTSecGrpIncByIdIdx( getPKey(), false );
		return( (ICFSecTSecGrpIncObj)retobj );
	}

	@Override
	public ICFSecTSecGrpIncObj read( boolean forceRead ) {
		ICFSecTSecGrpIncObj retobj = ((ICFBamSchemaObj)getSchema()).getTSecGrpIncTableObj().readTSecGrpIncByIdIdx( getPKey(), forceRead );
		return( (ICFSecTSecGrpIncObj)retobj );
	}

	@Override
	public ICFSecTSecGrpIncTableObj getTSecGrpIncTable() {
		return( ((ICFBamSchemaObj)getSchema()).getTSecGrpIncTableObj() );
	}
	...
	public ICFSecTSecGrpInc getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFSecBackingStore().getTableTSecGrpInc().readDerivedByIdIdx( ((ICFBamSchemaObj)getSchema()).getAuthorization(),
						getPKey() );
				if( rec != null ) {
					copyRecToPKey();
				}
	...
	public ICFSecTSecGrpIncEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFBamSchemaObj)getSchema()).getTSecGrpIncTableObj().lockTSecGrpInc( getPKey() );
		}
		edit = ((ICFBamSchemaObj)getSchema()).getTSecGrpIncTableObj().newEditInstance( lockobj );
	...
	public ICFSecTenantObj getRequiredOwnerTenant( boolean forceRead ) {
		if( ( requiredOwnerTenant == null ) || forceRead ) {
			...
			if( ! anyMissing ) {
				requiredOwnerTenant = ((ICFBamSchemaObj)getSchema()).getTenantTableObj().readTenantByIdIdx( getTSecGrpIncRec().getRequiredTenantId(), forceRead );
			}
		}
	...
	public ICFSecTSecGroupObj getRequiredContainerGroup( boolean forceRead ) {
		if( ( requiredContainerGroup == null ) || forceRead ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				requiredContainerGroup = ((ICFBamSchemaObj)getSchema()).getTSecGroupTableObj().readTSecGroupByIdIdx( getTSecGrpIncRec().getRequiredTSecGroupId(), forceRead );
			}
		}
	...
	public ICFSecTSecGroupObj getRequiredParentSubGroup( boolean forceRead ) {
		if( ( requiredParentSubGroup == null ) || forceRead ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				requiredParentSubGroup = ((ICFBamSchemaObj)getSchema()).getTSecGroupTableObj().readTSecGroupByIdIdx( getTSecGrpIncRec().getRequiredIncludeGroupId(), forceRead );
			}
		}
