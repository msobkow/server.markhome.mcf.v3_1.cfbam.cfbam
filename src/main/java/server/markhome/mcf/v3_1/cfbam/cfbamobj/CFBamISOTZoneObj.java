	...
	implements ICFBamISOTZoneObj
{
	...
	public ICFSecISOTZoneObj realise() {
		ICFSecISOTZoneObj retobj = ((ICFBamSchemaObj)getSchema()).getISOTZoneTableObj().realiseISOTZone(
	...
	@Override
	public void forget() {
		((ICFBamSchemaObj)getSchema()).getISOTZoneTableObj().reallyDeepDisposeISOTZone( (ICFSecISOTZoneObj)this );
	}

	@Override
	public ICFSecISOTZoneObj read() {
		ICFSecISOTZoneObj retobj = ((ICFBamSchemaObj)getSchema()).getISOTZoneTableObj().readISOTZoneByIdIdx( getPKey(), false );
		return( (ICFSecISOTZoneObj)retobj );
	}

	@Override
	public ICFSecISOTZoneObj read( boolean forceRead ) {
		ICFSecISOTZoneObj retobj = ((ICFBamSchemaObj)getSchema()).getISOTZoneTableObj().readISOTZoneByIdIdx( getPKey(), forceRead );
		return( (ICFSecISOTZoneObj)retobj );
	}

	@Override
	public ICFSecISOTZoneTableObj getISOTZoneTable() {
		return( ((ICFBamSchemaObj)getSchema()).getISOTZoneTableObj() );
	}
	...
	public ICFSecISOTZone getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFSecBackingStore().getTableISOTZone().readDerivedByIdIdx( ((ICFBamSchemaObj)getSchema()).getAuthorization(),
						getPKey() );
				if( rec != null ) {
					copyRecToPKey();
				}
	...
	public ICFSecISOTZoneEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFBamSchemaObj)getSchema()).getISOTZoneTableObj().lockISOTZone( getPKey() );
		}
		edit = ((ICFBamSchemaObj)getSchema()).getISOTZoneTableObj().newEditInstance( lockobj );
